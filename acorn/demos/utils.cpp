...
// Function to get data directory relative to current working directory
std::string getTestingDataDir() {
...
float* fvecs_read(const char* fname, size_t* d_out, size_t* n_out) {
    ...
    if (!f) {
        fprintf(stderr, "could not oopen %s\n", fname);
        perror("");
        abort();
    }
    int d;
...
std::vector<int> load_aq(
    ...
    if (dataset == "sift1M" || dataset == "sift1B") {
        ...
        // Compose File Name
        std::stringstream filepath_stream;
        filepath_stream << getTestingDataDir() << "/query_filters_sift"
        ...
        std::stringstream filepath_stream;
        filepath_stream << getTestingDataDir()
...
std::vector<int> load_ab(
    ...
    if (dataset == "sift1M" || dataset == "sift1B") {
        std::stringstream filepath_stream;
        filepath_stream << getTestingDataDir() << "/base_attrs_sift"
                        << (int)(N / 1000 / 1000) << "m_nc=" << n_centroids
        ...
        std::stringstream filepath_stream;
        filepath_stream << getTestingDataDir() << "/sift_attr" << ".json";
        std::string filepath = filepath_stream.str();
        // printf("%s\n", filepath.c_str());
        ...
        std::stringstream filepath_stream;
        filepath_stream << getTestingDataDir() << "/paper_attr.json";
        std::string filepath = filepath_stream.str();
        // printf("%s\n", filepath.c_str());
        ...
        std::stringstream filepath_stream;
        filepath_stream << getTestingDataDir() << "/base_attrs_tripclick.json";
        std::string filepath = filepath_stream.str();
        // printf("%s\n", filepath.c_str());
...
std::vector<int> load_attrb(std::string dataset, bool is_train) {
    std::string attr_type;
    if (is_train) {
        attr_type = "base_attr.json";
    } else {
        attr_type = "query_attr.json";
    }
    // Compose File Name
    std::stringstream filepath_stream;
    filepath_stream << getTestingDataDir() << "/" << dataset << "/"
                    << attr_type;
    std::string filepath = filepath_stream.str();
    printf("%s\n", filepath.c_str());
    std::vector<int> v = load_json_to_vector<int>(filepath);
