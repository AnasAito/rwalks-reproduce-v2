...
void shrink_neighbor_list(
    ...
    // ACORN::shrink_neighbor_list(qdis, resultSet, returnlist, max_size, gamma,
    // q_id, q_attr);
...
int hybrid_greedy_update_nearest(
        ...
        float& d_nearest) {
    debug("%s\n", "reached");
    // printf("hybrid_greedy_update_nearest called with parameters: filter: %d,
    // op: %d, regex: %s, level: %d\n", filter, op, regex.c_str(), level);
    int ndis = 0;
    for (;;) {
        int num_found = 0;
        storage_idx_t prev_nearest = nearest;
        ...
        for (size_t i = begin; i < end; i++) {
            ...
            // for debugging if (debugSearchFlag) {
            //     neighbors_checked.push_back(std::make_pair(v, metadata));
            //     debug_search("------------checking neighbor: %d, metadata:
            //     %d, metadata & filter: %d\n", v, metadata, metadata &
            //     filter);
            // }
            ...
            // check_regex result: %d\n", op, hnsw.metadata_strings[v].c_str(),
            // regex.c_str(), CHECK_REGEX(hnsw.metadata_strings[v], regex));
            if (filter_map[v]) {
            ...
            if (filter_map[v]) {
                ...
                if (dis < d_nearest || !filter_map[nearest]) {
                    nearest = v;
                    d_nearest = dis;
                    // debug_search("----------------new nearest: %d, d_nearest:
                    // %f\n", nearest, d_nearest);
                }
                if (num_found >= hnsw.M) {
...
#pragma omp critical
    ...
    // backtracking things
    std::vector<storage_idx_t> ep_per_level(
            max_level + 1); // idx of nearest node per level (size must be
                            // max_level+1 to include index max_level)
...
int hybrid_search_from_candidates(
        ...
        const SearchParametersACORN* params = nullptr) {
    // debug("%s\n", "reached");
    // printf("----hybrid_search_from_candidates called with filter: %d, k: %d,
    // op: %d, regex: %s\n", filter, k, op, regex.c_str());
    // debug_search("----hybrid_search_from_candidates called with filter: %d,
    // k: %d\n", filter, k);
    int nres = nres_in;
    int ndis = 0;
    ...
    while (candidates.size() > 0) { // candidates is heap of size max(efs, k)
        ...
        if (do_dis_check) {
            ...
            if (n_dis_below >= efSearch) {
                ...
                // reached"); debug_search("--------n_dis_below: %d, efSearch:
        ...
        for (size_t j = begin; j < end; j++) {
            ...
            // debug_search("------------visiting neighbor (%ld) - %d, metadata:
            // %d\n", j-begin, v1, metadata);
            ...
            // if (debugSearchFlag) {
            //     neighbors_checked.push_back(std::make_pair(v1, metadata)); //
            //     for debugging
            // }
            if (filter_map[v1]) {
            ...
            if (filter_map[v1]) {
                ...
                if (!sel || sel->is_member(v1)) {
                    if (nres < k) {
                        // debug_search("-----------------pushing new candidate,
                        // nres: %d (to be incrd)\n", nres);
                        faiss::maxheap_push(++nres, D, I, d, v1);
                        // debug_search("-----------------pushed new candidate,
                        // nres: %d\n", nres);
                        promising = 1;
                    } else if (d < D[0]) {
                        // debug_search("-----------------replacing top, nres:
                        // %d\n", nres);
                        faiss::maxheap_replace_top(nres, D, I, d, v1);
                        promising = 1;
                    }
                ...
                if (num_found >= hnsw.M * 2) {
                    ...
                    // triggered outer brea, skpping to M_beta=%d neighbork\n",
...
ACORNStats ACORN::hybrid_search(
    ...
    if (upper_beam == 1) { // common branch
        ...
        for (int level = max_level; level >= 1; level--) {
            ...
            // ndis_upper += hybrid_greedy_update_nearest(*this, qdis, filter,
            // op, regex, level, nearest, d_nearest);
            debug_search(
